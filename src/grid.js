...
function render(){
  ...
  for (let r=1;r<=ROWS;r++){
    ...
    for (let c=1;c<=COLS;c++){
      ...
      if (m && !(m.r1===r && m.c1===c)) {
        td.hidden = true; // covered cell → hidden placeholder to keep structure
        tr.appendChild(td);
        continue;
      }
      // master cell: set spans
      if (m && m.r1===r && m.c1===c) {
...
function detectSeries(vals){
  ...
  if (numsOK && nums.length >= 2) {
    const step = nums[nums.length-1] - nums[nums.length-2];
    return { kind:'number', step: Number.isFinite(step)? step : 0 };
  }
  // dates
  const dates = vals.map(parseMaybeDate);
  const datesOK = dates.every(v => v instanceof Date);
  if (datesOK && dates.length >= 2) {
    const stepMs = (+dates[dates.length-1]) - (+dates[dates.length-2]); // ✅ fixed bug: use dates.length
    const stepDays = Math.round(stepMs / 86400000) || 1;
    return { kind:'date', stepDays };
  }
  return null;
}
...
function attachGlobalShortcuts(){
  document.addEventListener('keydown', async (e)=>{
    ...
    // Arrow navigation when not focused in an input

...
export function setupExportImport(grid: GridAPI): void {
  // Export handlers
  ...
  // Import handlers
  const importers: [string, (buf: ArrayBuffer, grid: GridAPI) => void][] = [
...
function handleXLSX(buf: ArrayBuffer, grid: GridAPI): void {
...
function handleCSV(buf: ArrayBuffer, grid: GridAPI): void {
...
function handleTSV(buf: ArrayBuffer, grid: GridAPI): void {
  const text = new TextDecoder().decode(new Uint8Array(buf));
  const rows = text.replace(/\r/g, '').split('\n').map(r => r.split('\t'));
  grid.data = normalizeRect(rows);
}

function normalizeRect(aoa: any[][]): string[][] {
